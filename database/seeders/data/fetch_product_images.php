<?php

/**
 * One-off script: fetches a real, freely-licensed representative photo for
 * each product "image key" used by BulkProductSeeder, via Wikipedia's REST
 * summary API (returns official page thumbnails hosted on
 * upload.wikimedia.org). Run manually with `php fetch_product_images.php`.
 *
 * Output: product_images.php in this same directory, a plain
 * [image_key => image_url] array required by the seeder. Keys with no
 * resolvable thumbnail are simply omitted; the seeder falls back to the
 * category's own image in that case.
 */

$terms = [
    // Fruits
    'mango' => 'Mango',
    'banana' => 'Banana',
    'apple' => 'Apple',
    'guava' => 'Guava',
    'kinnow' => 'Kinnow',
    'sweet-lime' => 'Citrus limetta',
    'orange' => 'Orange (fruit)',
    'lemon' => 'Lemon',
    'peach' => 'Peach',
    'apricot' => 'Apricot',
    'plum' => 'Plum',
    'grapes' => 'Grape',
    'watermelon' => 'Watermelon',
    'muskmelon' => 'Cantaloupe',
    'pomegranate' => 'Pomegranate',
    'papaya' => 'Papaya',
    'pineapple' => 'Pineapple',
    'pear' => 'Pear',
    'dates' => 'Date palm',
    'litchi' => 'Litchi',
    'jamun' => 'Syzygium cumini',
    'chiku' => 'Manilkara zapota',
    'strawberry' => 'Strawberry',
    'fig' => 'Common fig',
    'falsa' => 'Grewia asiatica',
    'ber' => 'Ziziphus mauritiana',
    'coconut' => 'Coconut',

    // Vegetables
    'potato' => 'Potato',
    'tomato' => 'Tomato',
    'onion' => 'Onion',
    'cabbage' => 'Cabbage',
    'cauliflower' => 'Cauliflower',
    'spinach' => 'Spinach',
    'okra' => 'Okra',
    'eggplant' => 'Eggplant',
    'carrot' => 'Carrot',
    'radish' => 'Radish',
    'turnip' => 'Turnip',
    'peas' => 'Pea',
    'cucumber' => 'Cucumber',
    'bottle-gourd' => 'Calabash',
    'bitter-gourd' => 'Momordica charantia',
    'ridge-gourd' => 'Luffa acutangula',
    'pumpkin' => 'Pumpkin',
    'green-chili' => 'Chili pepper',
    'garlic' => 'Garlic',
    'ginger' => 'Ginger',
    'coriander' => 'Coriander',
    'mint' => 'Mentha',
    'spring-onion' => 'Scallion',
    'beans' => 'Green bean',
    'capsicum' => 'Bell pepper',
    'corn' => 'Maize',
    'beetroot' => 'Beetroot',
    'fenugreek' => 'Fenugreek',
    'taro' => 'Taro',
    'mustard-greens' => 'Mustard greens',
    'drumstick' => 'Moringa oleifera',
    'lettuce' => 'Lettuce',

    // Meat
    'chicken-as-food' => 'Chicken as food',
    'chicken-liver' => 'Chicken liver',
    'ground-meat' => 'Ground meat',
    'lamb-and-mutton' => 'Lamb and mutton',
    'beef' => 'Beef',
    'ground-beef' => 'Ground beef',

    // Sea food
    'rohu-fish' => 'Rohu',
    'katla-fish' => 'Catla',
    'hilsa-fish' => 'Ilisha',
    'catfish' => 'Catfish',
    'sole-fish' => 'Sole (fish)',
    'pomfret-fish' => 'Pomfret',
    'tilapia-fish' => 'Tilapia',
    'salmon-fish' => 'Salmon',
    'prawns' => 'Shrimp',
    'crab' => 'Crab',
    'squid' => 'Squid',
    'fish-as-food' => 'Fish as food',

    // Dairy
    'milk' => 'Milk',
    'yogurt' => 'Yogurt',
    'butter' => 'Butter',
    'cheese' => 'Cheese',
    'cream' => 'Cream',
    'ghee' => 'Ghee',
    'paneer' => 'Paneer',
    'lassi' => 'Lassi',
    'khoya' => 'Khoa',
    'cottage-cheese' => 'Cottage cheese',
    'whipped-cream' => 'Whipped cream',

    // Bakery
    'white-bread' => 'White bread',
    'whole-wheat-bread' => 'Whole wheat bread',
    'naan' => 'Naan',
    'bun' => 'Bun',
    'rusk' => 'Rusk',
    'sponge-cake' => 'Sponge cake',
    'chocolate-cake' => 'Chocolate cake',
    'biscuit' => 'Biscuit',
    'sheermal' => 'Sheermal',
    'nankhatai' => 'Nankhatai',
    'bakarkhani' => 'Bakarkhani',
    'doughnut' => 'Doughnut',

    // Drinks
    'apple-juice' => 'Apple juice',
    'orange-juice' => 'Orange juice',
    'cola' => 'Cola',
    'lemonade' => 'Lemonade',
    'bottled-water' => 'Bottled water',
    'tea' => 'Tea',
    'coffee' => 'Coffee',
    'rooh-afza' => 'Rooh Afza',
    'energy-drink' => 'Energy drink',

    // Pantry
    'basmati' => 'Basmati',
    'flour' => 'Flour',
    'sugar' => 'Sugar',
    'lentil' => 'Lentil',
    'vegetable-oil' => 'Vegetable oil',
    'mustard-oil' => 'Mustard oil',
    'salt' => 'Salt',
    'turmeric' => 'Turmeric',
    'chili-powder' => 'Chili powder',
    'garam-masala' => 'Garam masala',
    'cumin' => 'Cumin',
    'vermicelli' => 'Vermicelli',
    'gram-flour' => 'Gram flour',
    'chickpea' => 'Chickpea',
    'kidney-bean' => 'Kidney bean',

    // Frozen / quick meals
    'paratha' => 'Paratha',
    'chicken-nugget' => 'Chicken nugget',
    'french-fries' => 'French fries',
    'samosa' => 'Samosa',
    'seekh-kebab' => 'Seekh kebab',
    'ice-cream' => 'Ice cream',
    'spring-roll' => 'Spring roll',
    'biryani' => 'Biryani',
    'pilaf' => 'Pilaf',
    'karahi' => 'Karahi',
    'dal' => 'Dal',
    'sandwich' => 'Sandwich',
    'kathi-roll' => 'Kathi roll',
    'shawarma' => 'Shawarma',
    'nihari' => 'Nihari',

    // Home items
    'dishwashing-liquid' => 'Dishwashing liquid',
    'laundry-detergent' => 'Laundry detergent',
    'toilet-paper' => 'Toilet paper',
    'air-freshener' => 'Air freshener',
    'bin-bag' => 'Bin bag',
    'fabric-softener' => 'Fabric softener',
    'mosquito-repellent' => 'Mosquito repellent',

    // Kids
    'diaper' => 'Diaper',
    'wet-wipe' => 'Wet wipe',
    'infant-formula' => 'Infant formula',
    'baby-food' => 'Baby food',
    'lotion' => 'Lotion',
    'baby-powder' => 'Baby powder',

    // Self care
    'soap' => 'Soap',
    'shampoo' => 'Shampoo',
    'toothpaste' => 'Toothpaste',
    'toothbrush' => 'Toothbrush',
    'liquid-soap' => 'Liquid soap',
    'deodorant' => 'Deodorant',
    'razor' => 'Razor',
    'coconut-oil' => 'Coconut oil',

    // Gifts
    'chocolate' => 'Chocolate',
    'gift-basket' => 'Gift basket',
    'greeting-card' => 'Greeting card',
    'candle' => 'Candle',
    'perfume' => 'Perfume',
    'teddy-bear' => 'Teddy bear',

    // Flowers
    'rose' => 'Rose',
    'jasmine' => 'Jasmine',
    'marigold' => 'Tagetes',
    'lily' => 'Lilium',
    'tulip' => 'Tulip',
    'orchid' => 'Orchidaceae',
    'sunflower' => 'Common sunflower',
    'carnation' => 'Dianthus caryophyllus',

    // Pets
    'dog-food' => 'Dog food',
    'cat-food' => 'Cat food',
    'birdseed' => 'Birdseed',
    'fish-food' => 'Fish food',
];

$results = [];
$total = count($terms);
$i = 0;

foreach ($terms as $key => $title) {
    $i++;
    $url = 'https://en.wikipedia.org/api/rest_v1/page/summary/' . rawurlencode(str_replace(' ', '_', $title));

    $ch = curl_init($url);
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_TIMEOUT => 15,
        CURLOPT_HTTPHEADER => ['User-Agent: NavsonsSeederImageFetch/1.0 (dev@localhost)'],
        CURLOPT_SSL_VERIFYPEER => true,
    ]);
    $body = curl_exec($ch);
    $status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);

    $imageUrl = null;
    if ($status === 200 && $body) {
        $data = json_decode($body, true);
        $imageUrl = $data['originalimage']['source'] ?? $data['thumbnail']['source'] ?? null;
    }

    if ($imageUrl) {
        $results[$key] = $imageUrl;
        echo "[{$i}/{$total}] OK    {$key} => {$title}\n";
    } else {
        echo "[{$i}/{$total}] MISS  {$key} => {$title} (status {$status})\n";
    }

    // be polite to the API
    usleep(150000);
}

$export = "<?php\n\n// Auto-generated by fetch_product_images.php — do not edit by hand.\n// image_key => real photo URL (Wikimedia Commons, via Wikipedia REST summary API).\nreturn " . var_export($results, true) . ";\n";

file_put_contents(__DIR__ . '/product_images.php', $export);

echo "\nDone. Resolved " . count($results) . "/{$total} images.\n";
echo 'Written to ' . __DIR__ . "/product_images.php\n";
